...
PATH_DEFAULT = {
    "alaska": str(Path.home() / "data" / "ALASKA_v2_TIFF_512_COLOR"),
    "boss": str(Path.home() / "data" / "BOSSbase"),
}
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Know Your Library: comparison of libjpeg versions"
    )
    parser.add_argument(
        "mode",
        type=str,
        nargs="*",
        choices=["all", "compression", "decompression"],
        default="all",
    ...
    if args.input is not None:
        for inarg in args.input.strip().split(";"):
            ...
            assert dataset in {
                "alaska",
                "boss",
            }, "supported datasets are alaska and boss"
            if not path:
