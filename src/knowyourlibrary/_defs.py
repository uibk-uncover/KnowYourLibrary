...
cspaces = {1: jpeglib.JCS_GRAYSCALE, 3: jpeglib.JCS_RGB}
...
def compress_image(x: np.ndarray, path: str, ctx: TestContext):
    ...
    # compress
    im.write_spatial(
        path, qt=ctx.quality, dct_method=ctx.dct_method_compression, flags=flags
    )
...
def decompress_image(path: str, ctx: TestContext):
    ...
    flags = _flags[ctx.use_fancy_sampling]
    # decompress
    return jpeglib.read_spatial(
        path, dct_method=ctx.dct_method_decompression, flags=flags
    )
