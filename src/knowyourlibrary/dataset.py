...
def load_alaska_with_extremes(
    db_path: Path, sample_size: int, img_dimensions: Tuple[int, int]
) -> np.ndarray:
...
def get_checkerboard(
    boardsize: Tuple[int, int], tilesize: Tuple[int, int], channels: int = 3
) -> np.ndarray:
...
def generate_cropped_datasets(
    dataset: np.ndarray, offsets: list, base: tuple = (128, 128)
):
    # dimensions
