...
def run_compression_tests(dataset: np.ndarray):
    ...
    if dataset.shape[3] == 3:
        for use_fancy_sampling, method in zip(
            [True, False], ["fancy downsampling", "simple_scaling"]
        ):
            print(
                f"4:2:0 {method}",
            )
    ...
    def run_margin_compression_test(
        offsets, samp_factor=None, use_fancy_sampling=None, mod=8
    ):
    ...
    run_margin_compression_test([0, 1, 2, 4, 7, 8], ((1, 1), (1, 1), (1, 1)))
    if dataset.shape[3] == 3:
        for use_fancy_sampling, method in zip(
            [True, False], ["fancy downsampling", "simple_scaling"]
        ):
